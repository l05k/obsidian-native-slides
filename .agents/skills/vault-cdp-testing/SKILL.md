...
- **Never take the user's keyboard focus.** A capture raises and focuses the window it drives, and whatever the user is typing at that moment lands in the note the check just opened — in `example-vault/` that is a **tracked** fixture, so a stray keystroke becomes a dirty diff in someone else's working tree rather than a mere interruption. (It has happened: a run of captures put an accidental newline into `tests/typography-sample-media.md`.) So: say which window and moment you need, run the capture as **one burst**, and never loop captures while the user may be working. The rule has a technical edge too — a backgrounded or occluded window is throttled, so its rasterisation is **not reproducible** — and the answer to both is the **dedicated capture instance** described under _Settle before asserting_ below: it is not the maintainer's app, so it can be driven freely, and its throttling is disabled, so it renders deterministically while staying out of the foreground.
...
- **A capture runs in a dedicated instance, never the maintainer's.** Bring up a second Obsidian with its own `--user-data-dir` and port, holding only `example-vault/`, and point the check at it with `OBSIDIAN_CDP_PORT`:
  ...
  `-g` keeps the new window out of the foreground, so the human's typing never lands in a note the check opened; the three `--disable-*-throttling`/`-backgrounding` flags plus the occlusion feature disable are what make a **backgrounded** window render deterministically — without them a hidden window is throttled and consecutive captures of one tree differ by a few pixels of antialiasing at random. Measured with them: three full 14-shot runs byte-identical; without them: 1–3 stray pixels per run. The first launch of a fresh profile can report a stale version in its window title for a few seconds — wait for `state` to agree before capturing.
