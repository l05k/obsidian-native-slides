...
4. **Run checks before committing**:
   ...
   npm run check:scanner  # the community scanner's own two passes (see docs/development.md)
   ```
...
3. **CI must pass**: the `build` workflow runs `npm run check`, `npm test`, `npm run lint`, `npm run format:check`; the separate `scanner` job runs `npm run check:scanner:ts` (must be clean) and `npm run check:scanner:css` (the tracked `--max-warnings 24` baseline). Fix any failures before requesting review.
...
- **ESLint + Prettier** enforce style. Run `npm run lint:fix` and `npm run format` before committing. The community scanner's two passes are a separate gate: `npm run check:scanner`.
