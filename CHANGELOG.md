...
- **The community scanner's two passes now run in CI**, so a scorecard regression is caught on the pull request instead of on the plugin's store page. `npm run check:scanner` runs both locally: `eslint.obsidianmd.mjs` wraps the scanner's own `eslint-plugin-obsidianmd` preset over `main.ts` + `src/`, and `stylelint.config.mjs` extends the scanner's `stylelint-config-obsidianmd` over `styles.css` with nine non-scorecard rules and the two documented deviations silenced. The CSS pass carries a `--max-warnings 23` baseline — the two tracked follow-ups (#134/#135) — so a new warning fails; the TypeScript pass must be clean. `.github/workflows/ci.yml` runs both as a `scanner` job (#84).
...
- **17 of the 34 `!important` declarations are gone**, dropping the scanner's `declaration-no-important` count from 34 to 17 (the CSS pass's baseline moves from 44 to 27 in the same change, per the rule in `docs/development.md`). Verified by dumping the full computed style of all 1212 elements Slides mode renders across the seven fixtures — twice, byte-identical, then again after the change with **zero differences under both installed themes** (Minimal and Blue Topaz), with `slide-visual-check --tolerance 2` as the pixel backstop. The 17 that remain each have a measured reason, and they are not one story: 9 fight inline styles Obsidian writes (the card's `padding` and the eight list offsets), 1 the list pitch rule that loses on specificity, 1 the card-title `padding-top` that has to beat this file's own `!important` shorthand on `.cm-content`, 1 the quote's `border-left` that loses to Obsidian's own blockquote rule, 1 the pointer-hiding `cursor`, and 4 that only matter under a theme (the line width, the title's `margin-inline`, the H1 `letter-spacing`) — which is why a probe on one theme alone reports those as removable (#133).
...
- **Move slides in the slides panel**: drag an entry to a gap in the list — or use the context menu's **Move slide up** / **Move slide down**, shown as **Move N slides up** / **Move N slides down** when a selection is being moved — and the deck is reordered. A 2px accent line marks the gap the slide would land in (above the first entry and below the last one included), the entries being moved dim while a copy follows the pointer, `Escape` cancels the drag, and a drop that changes nothing writes nothing. Dragging an entry that belongs to the selection moves the whole selection as one block, keeping the relative order of the slides you did not pick; dragging one outside the selection moves just that slide and clears the selection. A move rewrites the `deck` links of the slides whose next link actually changed and touches nothing else, so the panel, the slides bar's page number and ◀ ▶ navigation follow the new order immediately (#124). Two consequences are documented in the README: a move writes frontmatter, which Obsidian's undo (`Cmd/Ctrl+Z`) does not cover, and the panel has to be visible — Slides mode hides both sidebars.

### Fixed

- **Card title stays on the card in themes that cap the readable line width**: with **Slides title = filename** and a theme that declares its own line width — Minimal's `--line-width: 40rem`, Blue Topaz's `--file-line-width: 45rem` — the title drifted off the card's text column and its bottom rule was clipped, by 93–324 px at the default 23 px font and further as the font grew. There were two independent causes, and both are fixed. A theme caps the card's **containing block** (Blue Topaz's `--file-line-width`), which shrinks the card and parks it against the pane's left edge, while the title is positioned against the sizer — so the two stopped describing one column. And a theme restyles the **title element itself** (Minimal sets `width: var(--line-width)`, `max-width`, and `margin-inline: … !important`), replacing the box the plugin positions with a 40rem box centred on the sizer. Slides mode now drops the readable-line-width cap on the card's containing block and pins the title's own width and margins, so the title's box and the card's text column resolve against boxes of the same width again. Property-backed titles were never affected (#125).
...
- **Next page no longer drops rapid presses or swaps the deck under you**: presses are queued and applied one open at a time, each anchored on the previous press's target, so a burst — key repeat, fast clicks on the bar button, or an impatient hand — advances one slide per press instead of collapsing into a single open (5 presses on a 6-slide deck used to advance one page, now they advance five, and presses past the last slide stay put). Navigation also stays inside the chain it started in: when two slides declare the same next slide (a shared `deck` link), stepping onto that slide used to re-derive the chain head from an arbitrary predecessor, so the reader was shown a different deck (landing on a slide labelled 3/4 after leaving 1/3) and _Previous page_ went to the other predecessor instead of back where they came from. A navigation session now remembers the chain **head** it entered and walks the chain live from there, and the bar, the slides panel and the navigation all resolve through that same walk — so a deck edited mid-session (a slide created, renamed or deleted) is reflected immediately rather than navigated into stale paths (#110).
...
[Unreleased]: https://github.com/l05k/obsidian-native-slides/compare/0.1.0...HEAD
[0.1.0]: https://github.com/l05k/obsidian-native-slides/releases/tag/0.1.0
