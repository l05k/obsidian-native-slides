...
| Move slides (drag, Move up/down)  | `planReorder()` (pure core) turns a moving set plus a gap into the `deck` rewrites it implies; `executeReorder()` writes them                                                                                                                |
