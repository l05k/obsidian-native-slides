...
| 移动幻灯片（拖动、Move up/down） | `planReorder()`（纯逻辑核心）把「移动哪些、插到哪个间隙」算成要改写的 `deck`；`executeReorder()` 写入                                                                                      |
...
**移动幻灯片（#124 起）。** 套件本身不存顺序——顺序**就是** next 链接链——所以一次移动只是重连被移动幻灯片周围的 `deck` 链接：`planReorder()`（纯逻辑核心）把「这些页、这个间隙」算成「每张 next 链接真的变了的页一条改写」，`DeckService.executeReorder()` 负责写入。也正因如此，翻页、页号与 slides 面板都不需要知道发生过移动（[ADR 0001](adr/0001-slide-order-is-the-next-link-chain.md)）。手势本体在 `src/panel-drag.ts`，基于 pointer 事件——4px 阈值让「按下」仍然是点击、浮动副本与间隙指示线、`Escape` 与窗口失焦取消、边缘自动滚动——因为 Obsidian 公开 API 里没有自定义视图可用的拖动重排能力（声明式设置列表的 `onReorder` 只在设置弹窗里渲染）。手势进行中链若已改变则放弃投放；不产生变化的投放不写盘。
...
这是插件持有的唯一会话状态，而且它是一个**链头提示，不是缓存下来的链**：链本身仍在每次解析时从 `metadataCache` 实时走一遍（原则 3），因此期间新建/重命名/删除的页都会被如实反映、也不会翻进已经失效的路径；而不再能到达当前页的提示会被直接忽略——不需要任何失效逻辑、也不需监听事件（原则 4）。它的理由是正确性而非性能：若没有它，`computeDeck()` 的回溯必须为「有多个前驱指向它」的那一页挑一个链头，这正是过去翻页中途套件换轨的原因。
