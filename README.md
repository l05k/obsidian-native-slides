...
- **Commands**: _Toggle Slides Mode_ (`Mod+Shift+E`), _Previous Page / Next Page_ (`Mod+Shift+←/→`), _Create Next Slide_ (`Mod+Shift+N`), _Create New Slide_, _Copy AI Agent Prompt_, _Show Slides Panel_, _Toggle Mouse Pointer_ (`Mod+Shift+M`), and _Toggle Slides Bar_ — all rebindable under _Settings → Hotkeys_.
