...
- **Slides panel**: a sidebar view that lists every slide of the active note's deck in chain order — click an entry to jump to it. Open it with the **Show Slides Panel** command or the presentation ribbon icon. Right-click an entry for **Create next slide** (insert after that slide) and **Delete slide**, or hold `Cmd/Ctrl` to select several slides and `Shift` to range-select, then right-click to delete the whole selection in one go — the deck chain is spliced automatically and deleted notes go to the trash (a confirmation dialog can be disabled under Settings → Confirm slide deletion).
