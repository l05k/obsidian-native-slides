...
- **Copy native slide skill for AI agent** command (inside Slides mode): measures the live layout and copies an AI-ready report to the clipboard — the real text area (minus slides bar and card title), how many lines and how many latin/CJK characters per line fit one screen, the measured line box of each element type (H1/H2/H3, body, bullet, code, first image), and capacity examples: "20 body lines", "H1 + 19 bullets", and so on. Ideal for telling an AI what fits before generating a slide. Follows the Obsidian UI language.
...
- **Commands**: _Toggle Slides Mode_ (`Mod+Shift+E`), _Previous Page / Next Page_ (`Mod+Shift+←/→`), _Create Next Slide_ (`Mod+Shift+N`), _Create New Slide_, _Copy Native Slide Skill For AI Agent_, _Show Slides Panel_, _Toggle Mouse Pointer_ (`Mod+Shift+M`), and _Toggle Slides Bar_ — all rebindable under _Settings → Hotkeys_.
