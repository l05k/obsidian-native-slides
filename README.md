...
- **Image embeds as centered card blocks** (settings, default on): in Slides mode, images render as centered blocks exactly as tall as the image — no reserved text-height bands, and text rows stack naturally around them. Turn the setting off to keep Obsidian's native inline flow (text flows beside a small image and wraps when the row runs out of width).
...
- **Commands**: _Toggle Slides Mode_ (`Mod+Shift+E`), _Previous Page / Next Page_ (`Mod+Shift+←/→`), _Create Next Slide_ (`Mod+Shift+N`), _Create New Slide_, _Show Slides Panel_, _Toggle Mouse Pointer_ (`Mod+Shift+M`), and _Toggle Slides Bar_ — all rebindable under _Settings → Hotkeys_.
