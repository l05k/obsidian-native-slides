...
- **[Development](docs/development.md)** ([简体中文](docs/development-zh.md)) — building the plugin (npm scripts), the dev loop with Obsidian reload, the dev-only typography debug tooling, the `src/` module layout, and the AI agent skills in `.agents/skills/` (this repo's own workflow skills plus the vendored mattpocock/skills set).
