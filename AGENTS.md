...
6. **Merge or stop** — `gh pr merge <pr> --squash`, then clean up **locally only** (see the harness note below): `git switch main` → `git branch -d <branch>` → `git pull origin main`. **Remote branches belong to the user** — never delete or prune one, and never hand over a command that would; the user deletes them on their own schedule. If a blocker is still open after the fixes, stop, leave the PR open, and hand it to the human with the findings; never merge over an open blocker. There is no second review: the fixes are covered by step 5 and by the round's PR comment, so a blocker found inside a fix means the PR goes to the human, not around the loop again.
...
1. **Branch / reference deletion is refused outright**: `git branch -d/-D`, `git branch --delete`, `git tag -d`, `git tag --delete`, `git push -d`, `git push --delete`, `git push origin :<branch>` and `gh pr merge --delete-branch` are denied before they run, with no authorization path in the hook — its deny text invites 先取得用户明确授权 ("first obtain the user's explicit authorization", a translation of the guard's Chinese), but no authorized retry can ever pass, so never attempt one. None of these needs a path: the guard attaches ref intent wherever the form appears, including to a command-shaped line of a heredoc body.

   **Standing instruction — local only, remotes are the user's:** the agent cleans up the branch it worked on **locally** and never touches a remote one. `gh api -X DELETE repos/<owner>/<repo>/git/refs/heads/<branch>` would technically run, because the guard attaches no ref intent to that REST form — which is exactly why the rule is stated rather than inferred from what happens to be blocked. Do not run it, and do not hand it to the user as a step to copy; the user deletes remote branches themselves, whenever they choose. What the agent does after a merge:
   ...
   If `-d` reports "not fully merged" (the remote branch was already deleted and pruned), stop and ask the user — `-D` is part of the same refused set. Never retry a refused deletion, and never `--delete-branch`. Why the local delete comes first: [`dev-workflow` §5](.agents/skills/dev-workflow/SKILL.md#5-after-the-pr-is-merged-clean-up-and-sync).
