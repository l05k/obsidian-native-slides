...
- **Driving the app must never take the user's keyboard focus.** A capture raises and focuses the window it drives, so a keystroke the user makes at that moment lands in the note the check just opened — and in `example-vault/` that is a tracked fixture, which turns their stray keystroke into a dirty diff in their tree. Announce which window and moment you need, run it as one burst, and leave the focus where you found it; never loop captures while the user may be working. A backgrounded window is throttled as well, so its rendering is not reproducible — a capture that needs the window visible needs a dedicated instance or an agreed moment, never a silent focus steal.
