...
- Driving the running app is fine — over CDP, for example — and this rule is what limits which vault it may touch. **The port is process-wide, so it can expose several windows, including one holding real notes**: identify the target by asking the app (`app.vault.adapter.getBasePath()`, never the window title) and abort on zero **or several** matches. `scripts/vault-cdp.mjs` is that guard — it refuses to connect unless exactly one window holds `example-vault/` — and never dispatch input to an element that has no layout (a collapsed sidebar gives every panel entry a `0×0` rect, and the click lands at `(0,0)` in the editor).
- **Never drive the instance the user is working in.** CDP picks its target by vault identity, not by OS focus, so if `example-vault/` is open in the window the user is _already_ typing in, the check's `openLinkText` swaps that window's active note and their next keystroke lands in the note the check just opened — a tracked fixture, which turns their stray keystroke into a dirty diff in their tree. A **separate and independent** reason: a backgrounded or occluded window is throttled by Chromium, so its rasterisation is not reproducible. Both are why a capture runs in a **dedicated instance** holding only `example-vault/`, never the user's own — the procedure, the launch command and the guard that enforces it are in [.agents/skills/vault-cdp-testing/SKILL.md](.agents/skills/vault-cdp-testing/SKILL.md).
