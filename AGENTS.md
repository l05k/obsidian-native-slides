...
1. **Branch / reference deletion is the user's, and the guard refuses it to the agent anyway**: `git branch -d/-D`, `git branch --delete`, `git tag -d`, `git tag --delete`, `git push -d`, `git push --delete`, `git push origin :<branch>` and `gh pr merge --delete-branch` are denied before they run, with no authorization path in the hook — its deny text invites 先取得用户明确授权 ("first obtain the user's explicit authorization", a translation of the guard's Chinese), but no authorized retry can ever pass, so never attempt one and never route around it. None of these needs a path: the guard attaches ref intent wherever the form appears, including to a command-shaped line of a heredoc body.
   ...
   `gh api -X DELETE repos/<owner>/<repo>/git/refs/heads/<branch>` is the one form that would technically run, because the guard attaches no ref intent to it — which is exactly why this is a **standing instruction** rather than a deduction from what happens to be blocked: the user deletes branches themselves, so do not run that either, and do not hand it over as a step to copy.

   **The agent's entire post-merge cleanup is a sync:**
   ...
   Do **not** run `git fetch --prune` before they have deleted the branch: pruning drops `origin/<branch>`, which after a squash merge is the only ref that lets their `git branch -d` pass — without it, only `-D` works. The warning they will see at that moment, and why the order is local-delete-then-prune, are explained in [`dev-workflow` §5](.agents/skills/dev-workflow/SKILL.md#5-after-the-pr-is-merged-the-branch-deletion-is-the-users).
