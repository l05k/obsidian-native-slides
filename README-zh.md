...
- **图片嵌入以居中卡片块呈现**（设置项，默认开）：Slides 模式下图片渲染为居中的卡片块，行高恰好等于图片本身——四周无预留的文字行空隙，文字行自然堆叠在其上下。关闭该设置则保持 Obsidian 原生 inline 流（小图与文字同行，行宽不足时才换行）。
